...
public class JFXTableView<S> extends TableView<S> {
    private ObservableList<S> itemSource;
    ...
    public JFXTableView(ObservableList<S> items) {
        super(items);
    ...
    protected void init() {
        ...
        this.filterMap.addListener((MapChangeListener<String, Predicate<S>>) change -> {
            if (filterMap.isEmpty()) this.setItems(itemSource);
            else
                filterMap.values().stream().reduce(Predicate::and)
                        .ifPresent(p -> this.setItems(new FilteredList<>(this.itemSource, p)));
        });
    }
